...
class Test_Meta_music_download():
    ...
    def test_1_1_1(self):
        # report path
        scan_meta_report_path = main_page.get_project_path('SFT/Report')+'/Download_meta_report.txt'
        ...
        # Get current Meta music items, Auto testing Scan music count = 100
        meta_list = media_room_page.background_music_show_meta_list(100)
        # result_fail_list = []
        for x in range(len(meta_list)):
            ...
            # Search music
            if current_music == 'No Pressure':
                media_room_page.search_library('Pressure')
            else:
                media_room_page.search_library(current_music)
            time.sleep(5)

            # Step3: Download music
            media_room_page.background_music_meta_context_menu_download(current_music)
